...
//cbu a string
    ...
    public override void Transferir(Cuenta cuentaDestino, decimal monto){
        if(ValidarTransferencia(monto)){
            Retirar(monto);
            CuentaMovimientos.Add(new Movimiento(monto,TipoMovimiento.TransferenciaEnviada));
            cuentaDestino.Depositar(monto);
            CuentaMovimientos.Add(new Movimiento(monto,TipoMovimiento.TransferenciaRecibida));
        }
    }
    ...
    public override void Retirar(decimal monto){
        if(ValidarRetiro(monto)){
            Saldo -= monto;
            CuentaMovimientos.Add(new Movimiento(monto,TipoMovimiento.Retiro));
        }
    }
